...
**`build_context` → `_build_vector_search_section`（Telegram / Discord 默认使用）**：先 `_build_rerank_query(session_id, user_message)`；**向量与 BM25 均以该多轮字符串为检索 query**；空字符串或调用方传 `skip_vector_search=True` 时跳过向量长期记忆块。跳过时本轮长期召回 trace 清空，远古 daily 概况补充也不会注入。Chroma `where` 与 BM25 `summary_type` 白名单仍只根据**当前用户句** `user_message`。后续为 `fuse_rerank_with_time_decay` + MMR（非 SiliconFlow Rerank API）。当前 idle 自主活动路径传 `skip_vector_search=True`，主对话默认不受影响。
...
**`build_context`（Telegram / Discord 主对话默认）**为双路检索 + 本地 `fuse_rerank_with_time_decay` + MMR，检索 query 为 `_build_rerank_query` 多轮字符串，**不经过** SiliconFlow Rerank API。调用方传 `skip_vector_search=True` 时跳过该块；当前 idle 自主活动路径使用该开关。**`build_context_async`（主服务未调用）**时长期记忆召回采用以下策略：
