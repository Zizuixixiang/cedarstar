...
**`build_context`（Telegram / Discord 主对话默认）**在 `rerank_enabled=true` 时采用以下策略；Rerank 配置优先来自 Mini App 核心设置里的 `rerank` 激活配置，失败则降级到本地 `fuse_rerank_with_time_decay` + MMR。`rerank_enabled=false` 或主路径异常回退时，旧链路以 `_build_rerank_query` 多轮字符串同时做向量和 BM25 检索，再本地融合排序。调用方传 `skip_vector_search=True` 时跳过该块；当前 idle 自主活动路径使用该开关。
