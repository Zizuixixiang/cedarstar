...
const TELEGRAM_CONFIG_ROWS = [
  ...
  },
  {
    ...
    name: '群聊每段字数上限（prompt 强制）',
    description: '写入 system：强硬要求每换行段≤该字数、每轮≤3 段；发送端不截断，靠模型自律',
    min: 10,
    max: 3800,
    step: 10,
  },
...
const CONFIG_METADATA = [
  ...
  {
    key: 'group_chunk_threshold',
    name: '群聊 Chunk 触发阈值',
    description: '群聊累计多少条消息触发一次日内微批总结',
    hint: '仅作用于群聊会话；留空则回退到普通 Chunk 阈值',
    min: 20,
    max: 100,
  },
  {
  ...
  {
    key: 'rerank_score_floor',
    name: 'Rerank 召回阈值',
    description: '非收藏事件通过 rerank 精排的最低分数门槛',
    hint: '低于此分数的候选直接丢弃，调低可增加召回量，默认 0.3',
    min: 0.05,
    max: 0.8,
    step: 0.05,
  },
  {
    key: 'rerank_starred_floor',
    name: 'Rerank 收藏阈值',
    description: '收藏事件通过 rerank 精排的最低分数门槛',
    hint: '收藏事件阈值更宽松，默认 0.15',
    min: 0.05,
    max: 0.5,
    step: 0.05,
  },
