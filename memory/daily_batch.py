...
class DailyBatchProcessor:
    ...
    def _daily_step2_session_framing(self, session_id: str) -> str:
        ...
        if is_group:
            return (
                f"以下材料汇总自 Telegram 群聊中当日产生的 chunk 摘要。chunk 对应助手人设「{cn}」；"
                f"用户侧主要对应「{un}」（群聊里只有三人，南杉、Sirius、Clio）。\n"
                f"抓住群内话题主线、重要事件、各方态度与情绪变化，以及关键事实（含数字、决策、名称、技术术语/报错信息、时间节点等），剔除语气词、重复内容。\n"
                f"区分不同说话人，避免把其他人的玩笑误写成「{un}」与助手之间的私密互动和既定事实。当提到其他助手的发言时，直接使用其名字称呼，不要使用「他」「对方」等模糊代词。"
            )
        return (
            "以下材料汇总自当日私聊产生的 chunk 摘要。\n"
            "撰写今日小传时聚焦双方关系中的核心话题、重要事件与情感变化，以及关键事实（含数字、决策、名称、技术术语/报错信息、时间节点等），剔除语气词、重复内容。"
        )
            ...
            if chunk_summaries:
                ...
                for session_id, session_chunks in grouped.items():
                    ...
                    step2_event_scope_bullet = (
                        "- 区分发生的事件和对话中提及的其他时间发生的事件，仅将本次群聊中实际发生的内容作为核心记录；提及的过往事件仅作上下文简要说明，例如「我提及X月X日……」，不得单独作为主要事件列出不可直接写成历史事实。"
                        if str(session_id).startswith("telegram_group_")
...
- 按主题归纳当日内容，并在主题内保持时间脉络；同一话题的多轮互动合并为一个事件段落，不逐条复述；只在话题切换或有明确时间跨度时标注时间点。不得遗漏核心讨论内容、重要决策、承诺和关键进展以及南杉需后续跟进的身体/情绪状态；日常互动在总结事件的基础上，保留能体现情感的因果逻辑和关键互动细节，不必逐句还原对话。
