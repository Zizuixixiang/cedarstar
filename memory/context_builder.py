...
async def format_telegram_reply_segment_hint() -> str:
    ...
    return (
        ...
        "- 按语气 / 停顿 / 情绪切分，N 段正文用 N-1 个 |||，总段数 ≤ MAX_MSG，禁止在句子中间截断\n"
        ...
        "[meme:…] 与 ||| 同为顺序分隔符，机器人按从左到右依次发出文字和表情包，"
        "可以插在多段文字之间，但不要因为用了[meme:…]就忽视 ||| 分隔。\n"
        "适合发的时机：调侃、吐槽、安慰、附和、庆祝、自嘲、共情等情绪浓度高的节点，"
        "或话说完需要一个视觉收尾的地方。\n"
        "不必每轮都发——去掉它对话没有损失就不发；连续两条都带表情包通常有一条是多余的。"
    )
