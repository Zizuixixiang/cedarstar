...
class MessageDatabase:
        ...
        for f in fields:
            if f == "enable_lutopia":
                ...
                try:
                    ...
                    values.append(0)
            elif f == "enable_ai_news_tool":
                raw = data.get("enable_ai_news_tool", 0)
                try:
                    values.append(1 if int(raw) else 0)
                except (TypeError, ValueError):
                    values.append(0)
    ...
    async def update_persona_config(self, persona_id: int, data: Dict[str, Any]) -> bool:
        ...
        if "enable_ai_news_tool" in update_data:
            try:
                update_data["enable_ai_news_tool"] = (
                    1 if int(update_data["enable_ai_news_tool"]) else 0
                )
            except (TypeError, ValueError):
