...
> **2026-04-26 追补（工具、群聊与图片上下文，以代码为准）：** 工具执行结果摘要会递归抽取 MCP / OpenAI-compatible 返回中的 `stdout`、`content[].text`、`data/result/output` 等正文写入 `tool_executions.result_summary`，Context 与 chunk 摘要继续只读短摘要；Telegram 图片追问规则扩展到“这张/那个/它/像不像/好看吗/哪里/这是什么”等短追问，并在本轮提示中注入最近图片摘要。图片 HTTP 400 兼容重试由 `llm/llm_interface._post_with_retry` 统一处理：GLM/Z.AI 流式请求也可重试裸 base64，OpenAI-compatible 流式图片可去掉 `stream_options`，必要时再尝试字符串 `image_url` 形态。群聊消息现在通过 HTTP peer relay 在 bot 间互传；用户单独 @ 某个 bot 时，未被 @ 的 bot 会忽略；群聊默认不显示思维链。另新增 `send_cot_in_group_chat` 与 `telegram_force_recent_images` 两个运行开关。
