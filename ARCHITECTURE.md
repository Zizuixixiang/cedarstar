...
**侧栏与导航（`App.jsx` / `sidebar.css` / `router.jsx`）：** **`navItems`** 每项除 **`Icon` / `text` / `path`** 外，可选 **`code`**（半透明等宽前缀，如 **`[ 01 ]`…`[ 06 ]`**、**`[ SYS ]`**）、可选 **`dividerBefore`**（在「助手配置」前插入**横向虚线 + ■**）。**`NavLink`** 选中态 **`.nav-item.active`**：**`#1A1A1A` 粗边框 + 右下硬阴影**，**`overflow: visible`** 以免 **`overflow: hidden`** 裁切阴影；侧栏 **右缘粗黑线 + 向右黑色硬阴影**。**路由入口：** `src/router.jsx` 导出 `navItems` 与 `routes`，文件顶部 `import React from 'react'`（见 §6.11）。
