...
后端路由分两类：`POST /api/mail/inbox` 在 `main.py` 单独挂载，只校验 `x-mail-secret`，不走 Mini App token；它解析 MIME 的 `from_addr / from_name / subject / body` 后先查 `mail_contacts`，发件人邮箱不存在时返回 200 并丢弃，存在时才写 `mail_inbox`，并后台用 `SummaryLLMInterface` 生成 100–200 字摘要写 `summary`，同时向 `TELEGRAM_MAIN_USER_CHAT_ID` 发送“收到来自 … 的新信件”提醒。`GET /api/mail/inbox`、`GET /api/mail/thread`、`POST /api/mail/outbox`、`GET/POST/DELETE /api/mail/contacts` 挂在受保护的 `/api/mail` 下，需 `X-Cedarstar-Token`。

内部工具 `read_mail(contact_email?, recent_n=3)` 查询 `mail_inbox` 与 `mail_outbox`，按时间升序合并，最近 `recent_n` 封带 `body` 原文，更早只带 `summary`。`send_mail(to_addr, to_name?, subject?, body)` 写入 `mail_outbox(status='pending')` 并创建 `pending_approvals.tool_name='send_mail_outbox'`；审批通过后调用 Resend API，成功后写 `status='sent'` 与 `sent_at`，并后台生成 outbox 摘要。邮件工具的 system 后缀包含写信规则与禁用句式。
