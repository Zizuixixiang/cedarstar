...
> 生成时间：2026-03-22（后续随代码演进修订，含 2026-04 Telegram / `save_message` 等与实现对齐说明）
...
- 消息长度限制 4096 字符（自动分割）。**缓冲回复（OpenAI 兼容主路径）：** SSE 流式编辑思维链占位消息（约 **0.45s** 节流），结束时**定稿为单独一条**消息（`<blockquote expandable>🧠 思维链`…，`parse_mode=HTML`）；若 **`edit_message_text(HTML)`** 失败则 **WARNING** 并尝试**删占位**后以 **`reply_text`** 重发同内容（内文去 `\x00` 以降低实体解析失败概率）。随后按 **`parse_telegram_segments_with_memes`** 将 **`|||`** 与 **`[meme:…]`** 拆成有序段，**交替**发送 HTML 正文与表情包（非「全文后发完再逐条发图」）。**非缓冲路径**（`_generate_reply`）：可选传入 **`telegram_bot`** 时同样先发思维链，再按上述有序段交付
