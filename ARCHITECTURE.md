...
│   ├── context_builder.py          # Context 组装器（system + … + 近期消息）；近期 **`assistant`** 经 **`strip_lutopia_behavior_appendix`**；**`user`** 条经 **`inject_user_sent_at_into_llm_content`** 注入东八区 **`【当前时间：…】`**（仅 LLM，不落库）；可选 **`telegram_segment_hint`**、**`tool_oral_coaching`**（Lutopia / 天气工具口播引导）
...
| `LLM_STREAM_READ_TIMEOUT_TOOLS_FLOOR` | 可选。当 **`generate_stream`** 请求**携带 `tools`**（如 Lutopia **`OPENAI_LUTOPIA_TOOLS`** ± 天气 **`OPENAI_WEATHER_TOOLS`**）时，与 **`LLM_STREAM_READ_TIMEOUT`** 取 **`max`** 作为实际读超时**下限**；默认 **180** 秒，缓解多轮 MCP 后上下文变长、模型两包间隔变长导致的误判超时 |
...
占位项在根目录 **`README.md`** 中多已标注为「规划中」；`meme.py`、`lutopia.py`、`weather.py`、`prompts.py` 为已实现模块。
