...
| `get_recent_daily_digest` | 最近 N 个日历日（`days` 默认 **7**，最大 30）日摘要 digest：东八区**今天**返回原文，其余日期仅返回压缩摘要（≤300 字/天），与 idle 共用 `memory/daily_summary_compress.py` 进程内缓存；需完整日摘要原文时用 `get_recent_summaries(summary_type=daily)` |
