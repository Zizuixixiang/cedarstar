...
邮件数据表由 `memory/database.py` 启动迁移幂等创建，显式脚本为 `migrations/20260529_add_mail_tables.sql`，需在 `cedarclio_db` 与 `cedarstar_db` 各自确认。Cloudflare Worker 代码在 `cloudflare/mail-worker.js`：`MAIL_ALLOWLIST` 为逗号分隔白名单，`message.from` 不在白名单时 `setReject`；`message.to` 为 `clio@cedarstar.org` 时转发到 `CEDARCLIO_INBOX_URL`，为 `sirius@cedarstar.org` 时转发到 `CEDARSTAR_INBOX_URL`；POST 原始 MIME，并带 `x-mail-secret: MAIL_SECRET`。`MAIL_SECRET` 与 `MAIL_ALLOWLIST` 用 `wrangler secret put` 设置，目标 URL 可用 Worker 环境变量设置。

后端路由分两类：`POST /api/mail/inbox` 在 `main.py` 单独挂载，只校验 `x-mail-secret`，不走 Mini App token；它解析 MIME 的 `from_addr / from_name / subject / body` 后写 `mail_inbox`，并后台用 `SummaryLLMInterface` 生成 100–200 字摘要写 `summary`，同时向 `TELEGRAM_MAIN_USER_CHAT_ID` 发送“收到来自 … 的新信件”提醒。`GET /api/mail/inbox`、`GET /api/mail/thread`、`POST /api/mail/outbox` 挂在受保护的 `/api/mail` 下，需 `X-Cedarstar-Token`。
...
待审批页（`/approvals`）展示来自内部记忆工具写入与 MCP `api_admin` 管理写入工具的 pending approval 请求，用户可在此批准或拒绝。该页另有「邮件」Tab，筛选 `tool_name='send_mail_outbox'` 的待发邮件并预览正文；「收件箱」Tab 调 `GET /api/mail/inbox` 展示 `mail_inbox` 列表与原文。
