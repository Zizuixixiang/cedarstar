...
用户发消息 → MessageBuffer.add_to_buffer() → 启动/重置 N 秒定时器
                          ...
                          合并 buffer 条目（文本 + 图片 payload）→ 调用各 bot 的 flush 回调
                                    ↓
                          build_context(..., images=..., llm_user_text=...) → LLM（多模态按 api_base/模型分流）→ 引用解析/hits/清洗 → 保存 → 可选异步视觉描述 → 触发微批检查
```
...
- 异步轮询：`Application.builder().token(...).build()` 后 `updater.start_polling(allowed_updates=Update.ALL_TYPES)`；**不在** `Application.builder()` 上链式 `.allowed_updates(...)`（部分已安装的 PTB 版本构建器无该 API，会导致启动失败）。`message_reaction` 等同属 `ALL_TYPES`，与下「Telegram 消息反应」一致
