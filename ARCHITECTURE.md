...
- 可转移错误：401、403、429、500/502/503/504、读超时与连接异常等。单渠道内 429/503 仍先走 `_post_with_retry`（最多 6 次、间隔 2s）。
...
- **下次触发时间（可选）**：触发句末尾会注入当前北京时间，并提示模型在回复末尾使用 `[NEXT_AT_HH:MM]`（24 小时制、东八区）。解析后写入 `idle_activity_next_trigger_at`（UTC ISO）；若该时刻已过则按次日处理。`check_and_trigger` 在 `idle_activity_enabled` 之后优先检查该键：未到点则跳过本次 tick；无标记或标记无效时清空该键，仍走原冷却 + 概率。发给用户的正文会剥除 `[NEXT_AT_...]` 标记。
...
- **API 激活池**：同 `config_type` 可多行「已启用」；`PUT /api/settings/api-configs/{id}/activate` 加入激活池、`PUT .../deactivate` 取消激活（见 **§2.3**）。删除前须先取消激活。
