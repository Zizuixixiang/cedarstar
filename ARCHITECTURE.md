...
- **人设开关**：`persona_configs.enable_rcommunity`；还须环境变量 **`RCOMMUNITY_MCP_TOKEN`**（`config.py` / `.env`）非空，实例才会建立 MCP SSE 会话。
...
- **调度**：`llm/llm_interface.py` 的 `complete_with_lutopia_tool_loop` 与 Lutopia 会话并列 `async with create_lutopia_mcp_session(), create_rcommunity_mcp_session()`；Telegram `bot/telegram_bot.py` 的 `_telegram_stream_thinking_and_reply_with_lutopia` 内 `append_tool_exchange_to_messages(..., rcommunity_mcp_session=...)`；Discord / idle 同走上述工具循环。`tool_executions` 照常记录；**rcommunity** 工具在 Telegram 流式 `execution_log` 中**不**拼入 Lutopia 旁白附录（见 `tools/lutopia.py` 排除逻辑）。
