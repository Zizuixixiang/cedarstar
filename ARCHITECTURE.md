...
- `[IDLE_TRIGGER]` 固定文案（`_IDLE_TRIGGER_TEXT`）中会列举可做的事，含在已启用工具时调用 `get_ai_news` 浏览 AI HOT 资讯/日报的提示（与人设 + `ENABLE_AI_NEWS_TOOL` 一致）。
...
- **人设开关**：`persona_configs.enable_rcommunity`；为真且环境变量 **`RCOMMUNITY_MCP_TOKEN`** 非空时，模型才有可能经工具链访问 rcommunity；**不会在首轮 `chat/completions` 之前**为 rcommunity 建立 MCP SSE。
- **连接**：默认基 URL `https://rcommunity-v2.rhysen.love/mcp`，实际请求为 `{base}?token={token}`；**不在** HTTP Header 或 MCP 参数中重复注入 token。可选 **`RCOMMUNITY_MCP_BASE_URL`** 覆盖基路径。
- **OpenAI 工具**：`OPENAI_RCOMMUNITY_TOOLS`（`rcommunity_forum` / `rcommunity_forum_write` / `rcommunity_forum_interact` / `rcommunity_chat` / `rcommunity_profile`）与 system 后缀 `RCOMMUNITY_TOOL_DIRECTIVE`（`tools/prompts.py`）。函数 `parameters` 使用显式 `request` 对象字段（内层 `additionalProperties: true`），执行前由 `_normalize_rcommunity_openai_args` 归一为传给 MCP 的平铺 dict（兼容模型仍输出顶层平铺 JSON 的情况）。MCP 返回序列化使用 `tools/mcp_utils.py` 的 `mcp_call_tool_result_to_json_str`（`tools/rcommunity.py` 顶部显式 import，与 Lutopia 共用实现）。
- **调度（懒加载 SSE）**：`llm/llm_interface.py` 的 `complete_with_lutopia_tool_loop` 与 Telegram `bot/telegram_bot.py` 的 `_telegram_stream_thinking_and_reply_with_lutopia` 使用 `contextlib.AsyncExitStack`：**首轮先调模型**；仅当本轮 `tool_calls` 中实际出现 Lutopia / rcommunity 工具名时，再 `enter_async_context(create_lutopia_mcp_session())` 与/或 `enter_async_context(maybe_rcommunity_mcp_session(True))`，避免 MCP 握手阻塞导致永远走不到上游 LLM。仍需 **`enable_rcommunity` 为人设真**才会为 rcommunity 建连（`maybe_rcommunity_mcp_session`）；仅配置 token 未开开关则不会建连。工具执行与回填仍经 `append_tool_exchange_to_messages(..., mcp_session=..., rcommunity_mcp_session=...)`；Discord / idle 走 `complete_with_lutopia_tool_loop` 的同一懒加载策略。对站方 ``call_tool`` 使用 ``asyncio.wait_for``（当前 **75s**）封顶，避免站方长时间不返回时拖死事件循环。
- **观测与附录**：`tool_executions` 照常记录；**rcommunity** 工具在 Telegram 流式 `execution_log` 中**不**拼入 Lutopia 旁白附录（见 `tools/lutopia.py` 排除逻辑）。
- **探测脚本**：`scripts/list_rcommunity_tools.py`（需已配置 token）。
...
`complete_with_lutopia_tool_loop` 采用「模型产出 tool_calls → 代码执行工具 → 将 tool 结果回填到 messages → 再次调用模型」的闭环方式。对模型而言，同一条工具链中的前序工具结果会保留在后续轮次上下文中，因此后续工具可以看到前一工具的执行结果。需要 Lutopia / rcommunity MCP 时，在**已得到含相应 tool_calls 的模型输出之后**再按需建立并复用各 SSE 会话（见 **§4.2.5**）；工具分发在循环内按函数名分支。
