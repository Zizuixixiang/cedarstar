...
│   ├── lutopia.py                  # Lutopia（论坛 `…/forum/api/v1` + Wiki `…/api/knowledge/…`）HTTP：`OPENAI_LUTOPIA_TOOLS`（对齐站方 AGENT_GUIDE：列表/单帖/评论树、编辑、帖/评投票、poll、activity、rename/avatar、submolts、Wiki 只读、`lutopia_knowledge`）、`get_lutopia_token`、`build_lutopia_behavior_appendix`、`execute_lutopia_function_call`（**`[tool]`** 日志）、发帖确认流、DM 与标已读等；供 `llm_interface` / Telegram Lutopia 路径调用
