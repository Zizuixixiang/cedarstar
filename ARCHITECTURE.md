...
`complete_with_lutopia_tool_loop` 采用「模型产出 tool_calls → 代码执行工具 → 将 tool 结果回填到 messages → 再次调用模型」的闭环方式。对模型而言，同一条工具链中的前序工具结果会保留在后续轮次上下文中，因此后续工具可以看到前一工具的执行结果。需要 Lutopia / rcommunity MCP 时，在**已得到含相应 tool_calls 的模型输出之后**再按需建立并复用各 SSE 会话（见 **§4.2.5**）；工具分发在循环内按函数名分支。Lutopia（`tools/lutopia.py`）与 rcommunity（`tools/rcommunity.py`）对站方 ``call_tool`` 均使用 ``asyncio.wait_for``（当前 **75s**）封顶，避免 MCP 读流挂起拖死事件循环。
