...
├── tools/                          # 工具函数层（部分实现）
│   ├── __init__.py                 # 包初始化文件
│   ├── meme.py                     # 表情包：`search_meme`（向量检索，可选 `top_k`）、`send_meme`（TG Bot 发静图/动图）；不由 LLM function calling 注册
...
- `tools/meme.py`：**`search_meme(query, top_k=…)`** 调 `meme_store` 向量检索；**`send_meme`** 为异步，需传入 Telegram `bot` 与 `chat_id`。不在 LLM 请求中注册为 tools；Telegram 在解析助手正文中的 **`[meme:…]`** 后调用（见 `bot/telegram_bot.py`、`bot/reply_citations.py`）
