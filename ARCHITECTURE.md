...
人设可单独控制：Lutopia、天气、微博热搜、网页搜索、X (Twitter)、**AI HOT 资讯**（`get_ai_news`）。记忆工具（`tools/memory_tools.py`）无条件加载，不受人设开关控制。**网页正文抓取 `web_fetch`**（`tools/web_fetch.py`）无人设开关，仅由部署环境 **`ENABLE_WEB_FETCH_TOOL`**（默认 true）控制是否注册；与记忆工具一样在开关为真时进入 OpenAI 兼容工具列表。工具口播提示由 system suffix 注入，确保模型在调用工具前先说一句自然口语。
