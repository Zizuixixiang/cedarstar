...
| 工具 | 说明 |
|---|---|
| `search_memories` | 向量 + BM25 双路召回搜索长期记忆，支持 type_filter、source_filter |
...
| 工具 | 说明 |
|---|---|
| `add_external_chunk` | 从网页端 Claude 整理的对话摘要写入记忆库。仅在用户明确说出「整理这个窗口」「写进记忆库」等显式指令时调用。LLM 拆分事件 → summaries 写 chunk 留底 → longterm_memories 逐条写事件 → ChromaDB embedding → BM25 |
...
| 外部写入 | MCP add_external_chunk 写入的 chunk 标记 source=claude_web，日终跳过重复聚类 |
