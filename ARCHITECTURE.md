...
| 工具 | 说明 |
|---|---|
| `add_external_chunk` | 从网页端 Claude 整理的对话摘要写入记忆库。仅在用户明确说出「整理这个窗口」「写进记忆库」等显式指令时调用。支持 `as_of_date` 参数补录历史窗口（30天内），补录后需手动触发 `trigger_daily_rerun` 重跑当日 daily。LLM 拆分事件 → summaries 写 chunk 留底（`[APP端]` 前缀） → longterm_memories 逐条写事件（`summary_type=app_event`） → ChromaDB embedding → BM25 |
