...
- Idle Activity 由进程内 `schedule_idle_activity_check()` 定时检查（当前 10 分钟一次）：当启用且满足时段、阈值、冷却和概率条件时，会注入 `[IDLE_TRIGGER]` 用户提示并调用 `complete_with_lutopia_tool_loop` 生成一条自主活动消息；该触发提示不落库，助手落库内容前缀为 `【自主活动】`
