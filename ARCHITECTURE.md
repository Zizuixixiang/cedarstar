...
│   ├── lutopia.py                  # Lutopia Forum HTTP：`OPENAI_LUTOPIA_TOOLS`、`get_lutopia_token`（`config.lutopia_uid`）、发帖确认流、删帖/删评论（`DELETE .../posts|comments/{id}`）、私信与收件箱等；供 `llm_interface` / Telegram Lutopia 路径调用
