...
>
> **2026-04-26（多模型图片、群聊、peer relay 与配置交互修正，以代码为准）：** OpenRouter 上的 Claude 不再因模型名含 `claude` 强制走 Anthropic `/messages`，而走 OpenAI-compatible `/chat/completions`；仅 **OpenRouter+Claude** 在该路径保留 text block `cache_control`，其它 OpenAI 兼容模型仍 flatten 并移除 Anthropic 专用字段。思维链提取补齐 `message.reasoning_content` / `reasoning` / `thinking` / content thinking blocks，覆盖 GLM / DeepSeek / Claude / Gemini 常见网关形态。群聊中已改为：普通群消息两名 bot 都可见；用户单独 @ 某个 bot 时，未被 @ 的 bot 会忽略；群聊默认不显示思维链，并通过 HTTP peer relay 在 bot 之间互传群聊助手消息。Telegram 分段在 Markdown ``` 代码围栏内不按换行或句末标点切分。`messages.platform_file_id` 支持 Telegram 图片历史重建；当前消息是否需要近期图片会先判断语义相关性，相关时才临时下载最近图片并附加到本轮 LLM 请求，另提供 `telegram_force_recent_images` 手动开关可直接强制传近期原图。新增 `group_chat_state`、群聊静默/互聊上限/插话概率配置与 `/silent` `/wake`；`assistant_other` 进入 Context 时包装为另一名助手发言。Mini App Config 支持 `send_cot_in_group_chat` 与 `telegram_force_recent_images` 开关，Settings 按 Base URL 分组 API 配置并支持模型收藏。详见 §3.2、§3.4.2、§5.1、§5.7、§5.11、§7.4。
...
> **2026-04-26（记忆系统 v3，以代码为准）：** **`api_configs.config_type`** 新增 **`analysis`**（与 `chat/summary/vision/stt/embedding/search_summary` 独立激活），供日终 **Step 4** 结构化事件拆分与 `score/arousal` 打分；未激活或加载失败时回退 `chat`。**Step 4** 不再把 daily 小传写入 ChromaDB，daily 仅保留在 PostgreSQL `summaries`，Chroma 新写入只包含 `daily_event` / `state_archive` / `manual`；旧 `daily` 向量不迁移、不删除。长期记忆召回移除父子折叠，改为双路召回 → `fuse_rerank_with_time_decay` → **MMR**（`config.mmr_lambda`）→ Top N。配置表新增 **`event_split_max`**（默认 8，1–15）与 **`mmr_lambda`**（默认 0.75，0.5–1.0）。
>
...
│   ├── weather.py                  # 天气：`fetch_weather_cached`（QWeather host: `HEFENG_API_HOST`/`QWEATHER_API_HOST`; key via `X-QW-Api-Key`; failures return unavailable/error payload, no mock）
...
│   ├── weather.py                  # 天气：`fetch_weather` / **`execute_weather_function_call`**（`get_weather`），返回 **JSON 对象字符串**；当 API 不可用时 summary 携带 error 原因
...
│       │   ├── Settings.jsx        # 核心设置页（API 配置管理：`chat` / `summary` / `vision` / `stt` / `embedding` / **`search_summary`** / **`analysis`** + Token 统计）
...
- 主对话 LLM：**Discord** 缓冲 flush 使用 `generate_with_context_and_tracking`。**Telegram 缓冲 flush**：OpenAI 兼容为 **SSE `generate_stream`**——无 Lutopia/天气/微博/**搜索**工具开关时为**单轮**、**`tools=None`**；**`enable_lutopia` 或 `enable_weather_tool` 或 `enable_weibo_tool` 或 `enable_search_tool`** 时为 **`_telegram_stream_thinking_and_reply_with_lutopia`**（**多轮**、**Lutopia ± 天气 ± 微博 ± 搜索** `tools` 按开关合并，见上条）；Anthropic 为 **`generate_with_context_and_tracking`（无 tools）**（见上条）。非缓冲 **`_generate_reply`** 为单次 **`generate_with_context_and_tracking`**；若传入 **`telegram_bot`** 则先发思维链，再与缓冲路径相同 **`await parse_telegram_segments_with_memes_async`** 后有序交替发文字与表情包。上述带 `usage` 的路径异步写入 `token_usage`（见 §3.3、§5.11）。**Telegram 流式路径 Token 补记（2026-04）：** SSE 子线程无 asyncio 事件循环，原先流式 usage 被丢弃；现已在主线程的 `done_payload` 中读取 `usage` 并调用 `llm._save_token_usage_async(u_data, Platform.TELEGRAM)`；Anthropic 与非缓冲路径亦在 `llm_resp.usage` 存在时调用 `llm._save_token_usage_async`，确保 Telegram 各路径均落库。`_assistant_outgoing_chunks` 仍保留（思维链转义 + 正文白名单净化） Concrete call shape: pass the raw usage dict to `_save_token_usage_async`; that wrapper normalizes usage and schedules `_async_save_token_usage(normalized_usage, platform)`.
...
- 支持 `config_type` 为 `chat` / `summary` / `vision` / **`search_summary`** / **`analysis`**（**`search_summary`** 供 **`tools/search.py`** 压缩 Tavily 原文，**`analysis`** 供日终 Step 4 结构化提取与打分，二者均与主对话 **`chat`** 独立激活；**语音转录 `stt` 不走本类**，由 **`bot/stt_client.py`** 单独读库调用 `/audio/transcriptions`）；**对话 API 路径**根据 `api_base` 是否含 `anthropic` 选择 Anthropic Messages API 与 OpenAI 兼容 `chat/completions`；用户多模态 content 按提供商组装（Claude：`image`+base64 source；OpenAI 兼容：`image_url`+data URL）
...
- **Token 统计：**仅当调用带 tracking 的方法且响应中含 `usage` 时才会写入 `token_usage`（见 §5.11）：若当前线程存在**正在运行的** asyncio 事件循环，则 `create_task` 走 `_async_save_token_usage`；否则（例如 `bot/vision_caption.py` 在 `run_in_executor` 线程内调 vision LLM）**同步**调用 `get_database().save_token_usage(...)`，避免 `no running event loop` 与未 await 的协程告警。`generate` / `generate_simple` / `generate_with_context` / `chat` **不会**落库用量。**`generate_stream`（SSE）** 在 payload 中加入 `stream_options: {"include_usage": true}`，使 OpenAI 兼容网关在末包携带 usage 数据；Telegram 缓冲 flush 的流式路径在主线程(`done_payload`) 收到 usage 后调用 `_save_token_usage_async(u_data, Platform.TELEGRAM)`（原子补记，绕过子线程无事件循环限制），Anthropic 与非缓冲路径亦同步补调 `_save_token_usage_async`。 Do not call `_async_save_token_usage(prompt_tokens, completion_tokens, total_tokens, platform)`; the coroutine accepts a normalized usage dict plus optional platform.
...
**精排（仅异步路径）：** 并行双路检索并按 `doc_id` 去重后，对候选调用 Cohere 得到语义相关分；对每条再算时间衰减复活分（`_memory_age_days`：**优先**用 metadata `last_access_ts` 计龄；**仅当缺失或无法解析时**用 `created_at` 兜底）：
...
两路分数各自在当批候选内 min-max 归一化后按 **0.8×语义 + 0.2×衰减** 综合得分排序，再执行 **MMR** 多样性筛选取 top **N** 写入 context（N=`config.context_max_longterm`，默认 **3**）。
...
- 同步版 `build_context()`：双路检索 + 融合分 + MMR，无 Cohere；长期记忆块标题为「双路检索结果」
- 异步版 `build_context_async()`：并行检索 + Cohere 全候选打分 + 上述融合公式 + MMR 取 top **N**（同上）；`COHERE_API_KEY` 不可用时回退为同步双路逻辑
...
| Step 4 | 使用 `analysis`（失败回退 `chat`）对 daily 小传整体打分，并强制拆出至少 1 个事件、最多 `event_split_max` 个事件；每个 event 输出 `summary` / `score` / `arousal`，`halflife_days`：8–10→600，4–7→200，1–3→30。daily 不再写 ChromaDB；仅写 `daily_{batch_date}_event_N`，metadata 含 `parent_id=daily_{batch_date}`，增量更新 BM25。analysis 连续 3 次失败后使用 score=5/arousal=0.1、Telegram 告警并继续 |
...
- **Step 4**（小传与事件 **score/arousal**）：优先使用激活 **`analysis`** 配置；未激活或加载失败回退 **`chat`**。事件拆分不再走摘要模型 `generate_summary`，而是 Step 4 analysis 调用并带三次重试、默认值兜底。
...
- **旧库 / 误建成 2048 的集合：** 若本地 `chroma_db` 已按错误维度写入，**处理（推荐）：先停止占用 Chroma 的进程**，备份后**删除** `chroma_db` 目录，确保代码已带 `dimensions: 1024` 后再启动并重新跑批写入。旧架构向量与当前 metadata / v3 event-only 新写入约定不一致时，重建通常比就地迁移更干净；`longterm_memories` 表中历史行可能变为 Chroma 侧「孤儿」，由 Mini App `is_orphan` 提示，可按需清理或重新录入。
...
- `memory/bm25_retriever.py` sets `jieba.dt.cache_file = "/opt/cedarstar/jieba.cache"` immediately after importing `jieba`; `/jieba.cache` is ignored by Git. This avoids `/tmp/jieba.cache` permission failures when batch jobs and services run under different users.
...
**职责：** 使用 Cohere `rerank-multilingual-v3.0` 模型对双路去重后的候选文档打分，供 `context_builder` 与半衰期衰减分融合；融合后再由 MMR 做多样性筛选。

**边界：**
- 纯异步实现（`cohere.AsyncClient`）
- 在 `build_context_async()` 中对去重后的候选调用，`top_n` 可为全量候选数以取齐语义分
...
**Settings 页（`Settings.jsx` / `settings.css`）：** 「对话 API」「摘要 API」「视觉 API」「语音转录」「**Embedding**」「搜索摘要」「分析 API」七个 Tab，列表分别请求 `GET /api/settings/api-configs?config_type=…`（`chat` / `summary` / `vision` / `stt` / `embedding` / `search_summary` / `analysis`），切换 Tab 时重新拉取。首次迁移会在 `api_configs` 插入默认 **`config_type=embedding`** 行（名称「硅基流动 bge-m3」、`base_url`/`model` 预填、`api_key` 空、**已激活**），用户在 Mini App 中补 Key 即可。新增/编辑弹窗内可改 `config_type`；**保存成功后以表单中的类型为准**——若与当前 Tab 不一致则自动切换到对应 Tab 并加载列表。`POST`/`PUT` 允许的 `config_type` 含 `embedding`、`search_summary`、`analysis`（分别独立激活）。**UI（以代码为准）：** API / Period 分类为**横向可滚动**的独立 neo-brutalist 按钮（**`.config-tabs` / `.period-tabs`**，隐藏滚动条）；配置行列表在**窄屏**为纵向堆叠，**`cfg-mid`** 等 **`min-width: 0`** 防挤压；大卡片 **`SETTINGS`**、行内 **`API CONFIG`** 角标配合 **`.settings-page` 与卡片内边距**避免视口裁切。Token 数字区为**双列紧凑网格**（**`.token-nums`**），平台占比（**`.platform-bars`**）为**弱化的细条**辅助信息，数据仍来自动态 **`tokenStats.by_platform`** 与 **`PLATFORM_COLOR`**（逻辑同 2026-04-07）。**Token 统计首次加载：** 挂载时 **`GET /api/settings/token-usage?period=latest`**。页标题图标：**`lucide-react`** 的 **`KeyRound` / `BarChart3`**。
...
- `tools/weather.py`：**`fetch_weather`** / **`execute_weather_function_call`**（**`get_weather`**），复用 **`api.weather.fetch_weather_cached`**；QWeather 请求使用 **`config.HEFENG_API_HOST`**（可由 **`HEFENG_API_HOST`** / **`QWEATHER_API_HOST`** 指定，默认 `devapi.qweather.com`）与 **`X-QW-Api-Key`**；缺少 key、城市未解析到 LocationID、上游失败时返回 **`{"summary":…, "error":…}`**，不再返回 mock 天气。**`role=tool`** 内容始终为 **JSON 对象字符串**，满足 Gemini 等网关对工具返回 **Struct** 的要求
...
cron（或同类）在运维约定时刻触发 —— 应与 `config.daily_batch_hour`（东八区半小时粒度，默认 23.0）一致
  ...
  ├─────────────────────────────────────────────────────┤
  │  Step 4：analysis 打分/拆事件 + event-only Chroma       │
...
```
  ...
  llm/llm_interface.py._load_active_config()
        │  从 api_configs 表读取 is_active=1 的配置（含 persona_id）；`config_type` 为 `chat` / `summary` / `vision` / `stt` / `embedding` / `search_summary` / `analysis` 时各自独立激活
        ▼
...
- `event_split_max`：Step 4 单日事件拆分软上限（默认 8，范围 1–15）
- `mmr_lambda`：长期记忆 MMR 相关性权重（默认 0.75，范围 0.5–1.0）
...
| `config_type` | TEXT | 配置类型（`chat` / `summary` / `vision` / `stt` / `embedding` / **`search_summary`** / **`analysis`**） |
| `created_at` | TIMESTAMP | 创建时间 |
| `updated_at` | TIMESTAMP | 更新时间 |
...
| **Settings.jsx** | `GET /api/settings/api-configs?config_type=chat|summary|vision|stt|embedding|search_summary|analysis`（按 Tab 过滤）、`POST` / `PUT` / `DELETE` / `PUT .../activate`、`POST .../fetch-models`、`GET /api/settings/token-usage`、`GET /api/persona`；保存配置后按返回表单中的 `config_type` 切换 Tab 或刷新当前列表（见 §3.6 Settings 页说明） |
