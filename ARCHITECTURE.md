...
- Mini App「工具中心 → MCP 管理」与 REST `api/custom_mcp.py` 维护 server、headers、开关和工具列表；路由挂载在 `/api/mcp`。列表页名称与启用徽章同一行，同步/保存失败提示显示在对应卡片旁；工具列表页展示已同步参数的摘要（未同步 schema 时提示重新同步）。
