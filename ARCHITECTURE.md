...
- **人设开关**：`persona_configs.enable_rcommunity`；为真且环境变量 **`RCOMMUNITY_MCP_TOKEN`** 非空时，工具循环内才会建立 MCP SSE。
...
- **OpenAI 工具**：`OPENAI_RCOMMUNITY_TOOLS`（`rcommunity_forum` / `rcommunity_forum_write` / `rcommunity_forum_interact` / `rcommunity_chat` / `rcommunity_profile`）与 system 后缀 `RCOMMUNITY_TOOL_DIRECTIVE`（`tools/prompts.py`）。
- **调度**：`llm/llm_interface.py` 的 `complete_with_lutopia_tool_loop` 与 Lutopia 会话并列进入；**仅当人设 `enable_rcommunity` 为真**时才 `maybe_rcommunity_mcp_session(True)` 内建立 rcommunity SSE（见 `tools/rcommunity.py`），避免仅配置了 token 却未开开关时每轮无谓建连。Telegram `bot/telegram_bot.py` 的 `_telegram_stream_thinking_and_reply_with_lutopia` 同理；`append_tool_exchange_to_messages(..., rcommunity_mcp_session=...)`；Discord / idle 同走上述工具循环。
- **观测与附录**：`tool_executions` 照常记录；**rcommunity** 工具在 Telegram 流式 `execution_log` 中**不**拼入 Lutopia 旁白附录（见 `tools/lutopia.py` 排除逻辑）。
- **公共 MCP 序列化**：`tools/mcp_utils.py` 的 `mcp_call_tool_result_to_json_str`（Lutopia / rcommunity 共用）。
...
`complete_with_lutopia_tool_loop` 采用「模型产出 tool_calls → 代码执行工具 → 将 tool 结果回填到 messages → 再次调用模型」的闭环方式。对模型而言，同一条工具链中的前序工具结果会保留在后续轮次上下文中，因此后续工具可以看到前一工具的执行结果。启用 rcommunity 时与 Lutopia MCP 各持一条 SSE 会话（见 **§4.2.5**），工具分发在循环内按函数名分支。
