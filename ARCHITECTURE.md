...
```
cedarstar/                          # 项目根目录
├── main.py                         # 主入口：校验配置 → 阻塞重建 BM25 → `setup_telegram_webhook_app()`（无 TG 轮询）→ 可选 `ENABLE_DISCORD` 时启动 Discord 线程 → FastAPI（含 `/webhook/telegram` + `/api/*`）；日终跑批不由此进程调度。setup_logging 含 httpx/httpcore 对 api.telegram.org 的 INFO 过滤（避免 Token 入日志）
