...
async def complete_with_lutopia_tool_loop(
    ...
    *,
    ...
    from tools.mail_tools import OPENAI_MAIL_TOOLS, execute_add_mail_contact, execute_list_mail_contacts, execute_read_mail, execute_send_mail
            ...
            for tc in last.tool_calls or []:
                ...
                try:
                    if nm == "get_weather":
                        ...
                        try:
                            args_add_contact: Dict[str, Any] = json.loads(raw_args or "{}")
                        except json.JSONDecodeError:
                            args_add_contact = {}
                        result_str = await execute_add_mail_contact(args_add_contact)
                    elif nm == "read_mail":
                        try:
                            args_mail_read: Dict[str, Any] = json.loads(raw_args or "{}")
                        except json.JSONDecodeError:
                            args_mail_read = {}
                        result_str = await execute_read_mail(args_mail_read)
                    elif nm == "send_mail":
                        try:
                            args_mail_send: Dict[str, Any] = json.loads(raw_args or "{}")
                        except json.JSONDecodeError:
                            args_mail_send = {}
                        result_str = await execute_send_mail(args_mail_send)
                    elif nm.startswith("game_"):
                        try:
                            args_game: Dict[str, Any] = json.loads(raw_args or "{}")
                        except json.JSONDecodeError:
                            args_game = {}
                        result_str = await execute_game_function_call(nm, args_game)
                    elif nm in (
